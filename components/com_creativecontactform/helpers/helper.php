...
class CreativecontactformHelper
{
	...
	private function print_fields_array_html($field_data) {
		...
		foreach($field_data as $field) {
			...
			elseif($field_type == 'contact-data') {
				...
				$processed_data = $this->process_txt($sections_html,array('sections','num','heading'));

				$input_html = $processed_data;
			}
			elseif($field_type == 'custom-html') {
				$custom_html = stripslashes($field['custom_html']);
				$processed_data = $this->process_txt($custom_html,array('sections','num','heading'));
	...
	public function render_html()
	{
		...
		$pretxt = $this->process_txt($pretxt,array('num'));
 				...
 				<form class="creativecontactform_form">
		 			<div class="creativecontactform_body <?php echo $body_animation_class;?>">
			 				...
			 				if($send_copy_enable == 1) { 
			 					// $field_index ++;
			 					echo '<div class="creativecontactform_clear"></div><div class="creativecontactform_field_box creative_hidden_animation_block_state1 creative_timing_sendmecopy creative_timing_6 '.$field_box_animation_class.'"><div class="creativecontactform_field_box_textarea_inner">';
				 					echo '
